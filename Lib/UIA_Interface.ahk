...
/* Questions:
	...
	- if method returns a SafeArray, should we return a Wrapped SafeArray, Raw SafeArray, or AHK Array. Currently we return wrapped AHK arrays for SafeArrays that contain elements, and raw SafeArrays for everything else. Reason: SafeArrays are more convenient to loop over (for key in SafeArray). UIA_SafeArrayToNativeArray can be used to convert to AHK array.
...
class UIA_Interface extends UIA_Base {
	...
	CreatePropertyCondition(propertyId, var, type="Variant") {
		...
		else if (maybeVar := UIA_PropertyVariantType(propertyId)) {
			UIA_Variant(var,maybeVar,var)
		}
		return UIA_Hr((A_PtrSize == 4) ? DllCall(this.__Vt(23), "ptr",this.__Value, "int",propertyId, "int64", NumGet(var, 0, "int64"), "int64", NumGet(var, 8, "int64"), "ptr*",out) : DllCall(this.__Vt(23), "ptr",this.__Value, "int",propertyId, "ptr",&var, "ptr*",out))&&!UIA_VariantClear(var)? new UIA_PropertyCondition(out):
	}
	CreatePropertyConditionEx(propertyId, var, type="Variant", flags=0x1) {
		...
		return UIA_Hr((A_PtrSize == 4) ? DllCall(this.__Vt(24), "ptr",this.__Value, "int",propertyId, "int64", NumGet(var, 0, "int64"), "int64", NumGet(var, 8, "int64"), "uint",flags, "ptr*",out) : DllCall(this.__Vt(24), "ptr",this.__Value, "int",propertyId, "ptr",&var, "uint",flags, "ptr*",out))&&!UIA_VariantClear(var)? new UIA_PropertyCondition(out):
	}
	...
	*/
	...
	AddAutomationEventHandler(eventId, element, scope=0x4, cacheRequest=0, handler="") {
		return UIA_Hr(DllCall(this.__Vt(32), "ptr",this.__Value, "int", eventId, "ptr", element.__Value, "uint", scope, "ptr",cacheRequest.__Value,"ptr",handler))
	}
	RemoveAutomationEventHandler(eventId, element, handler) {
		return UIA_Hr(DllCall(this.__Vt(33), "ptr",this.__Value, "int", eventId, "ptr", element.__Value, "ptr",handler))
	}
	...
	AddPropertyChangedEventHandler(element,scope=0x1,cacheRequest=0,handler="",propertyArray="") {
		...
		return UIA_Hr(DllCall(this.__Vt(35), "ptr",this.__Value, "ptr",element.__Value, "int",scope, "ptr",cacheRequest.__Value,"ptr",handler,"ptr",ComObjValue(SafeArray)))
	}
	RemovePropertyChangedEventHandler(element, handler) {
		return UIA_Hr(DllCall(this.__Vt(36), "ptr",this.__Value, "ptr",element.__Value, "ptr", handler))
	}

	AddStructureChangedEventHandler(element, handler) { ; UNTESTED
		return UIA_Hr(DllCall(this.__Vt(37), "ptr",this.__Value, "ptr",element.__Value, "ptr",handler))
	}
	RemoveStructureChangedEventHandler(element, handler) { ; UNTESTED
		return UIA_Hr(DllCall(this.__Vt(38), "ptr",this.__Value, "ptr", element.__Value, "ptr",handler))
	}

	AddFocusChangedEventHandler(cacheRequest=0, handler="") { ; handler is required, cacheRequest can be left to 0
		return UIA_Hr(DllCall(this.__Vt(39), "ptr",this.__Value, "ptr",cacheRequest.__Value, "ptr",handler))
	}
	RemoveFocusChangedEventHandler(handler) {
		return UIA_Hr(DllCall(this.__Vt(40), "ptr",this.__Value, "ptr",handler))
	}
	...
	*/
	...
	CreateCondition(property, val, flags=0) { ; flags: 0=no flags; 1=ignore case; 2=match substring; 3=ignore case and match substring
		if !RegexMatch(property, "^\d+$")
			RegexMatch(property, "(?:UIA_)?\K.+?(?=(Id)?PropertyId|$)", property), propCond := UIA_PropertyId(property), property := StrReplace(StrReplace(property, "AnnotationAnnotation", "Annotation"), "StylesStyle", "Style")
		else
			propCond := property
		if RegexMatch(val, "^\w+$") {
			val := IsFunc("UIA_" property "Id") ? UIA_%property%Id(val) : IsFunc("UIA_" property) ? UIA_%property%(val) : val
		}
		if (propCond && val)
...
class UIA_Interface3 extends UIA_Interface2 {
	...
	AddTextEditTextChangedEventHandler(element, scope, textEditChangeType, cacheRequest=0, handler="") {
		return UIA_Hr(DllCall(this.__Vt(64), "ptr",this.__Value, "ptr", element.__Value, "int", scope, "int", textEditChangeType, "ptr", cacheRequest.__Value, "ptr", handler))
	}
	RemoveTextEditTextChangedEventHandler(element, handler) {
		return UIA_Hr(DllCall(this.__Vt(65), "ptr",this.__Value, "ptr", element.__Value, "ptr", handler))
	}
}
...
class UIA_Interface4 extends UIA_Interface3 {
	...
	AddChangesEventHandler(element, scope, changeTypes, changesCount, cacheRequest=0, handler="") {
		return UIA_Hr(DllCall(this.__Vt(66), "ptr",this.__Value, "ptr", element.__Value, "int", scope, "int", changeTypes, "int", changesCount, "ptr", cacheRequest.__Value, "ptr", handler))
	}
	RemoveChangesEventHandler(element, handler) {
		return UIA_Hr(DllCall(this.__Vt(67), "ptr",this.__Value, "ptr", element.__Value, "ptr", handler))
	}
}
class UIA_Interface5 extends UIA_Interface4 {
	...
	AddNotificationEventHandler(element, scope, cacheRequest, handler) {
		return UIA_Hr(DllCall(this.__Vt(68), "ptr",this.__Value, "ptr", element.__Value, "int", scope, "ptr", cacheRequest.__Value, "ptr", handler))
	}
	RemoveNotificationEventHandler(element, handler) {
		return UIA_Hr(DllCall(this.__Vt(69), "ptr",this.__Value, "ptr", element.__Value, "ptr", handler))
	}
}
...
class UIA_Element extends UIA_Base {
	...
	;~ FindFirstBuildCache 	7	IUIAutomationElement UNTESTED
	FindFirstBuildCache(c="", scope=0x4, cacheRequest="") {
	...
	;~ FindAllBuildCache 	8	IUIAutomationElementArray UNTESTED
	FindAllBuildCache(c="", scope=0x4, cacheRequest="") {
	...
	;~ GetCachedPropertyValue 12 UNTESTED
	...
	GetCachedPatternAs(pattern="", ByRef usedPattern="") {
		...
		;	return UIA_Hr(DllCall(this.__Vt(15), "ptr",this.__Value, "int",pId, "ptr",UIA_GUID(riid,iid), "ptr*",out))&&out? new UIA_%pattern%Pattern(out):
		if (InStr(usedPattern:=pattern, "Pattern")||(usedPattern := UIA_Pattern(pattern)))
	...
	GetCurrentPattern(patternId) {
		return UIA_Hr(DllCall(this.__Vt(16), "ptr",this.__Value, "int",patternId, "ptr*",out))
		if name:=UIA_PatternId(patternId)
			return new UIA_%name%(out)
		return out
	}
	;~ GetCachedPattern 	17	Iunknown **patternObject
	GetCachedPattern(patternId) {
		return UIA_Hr(DllCall(this.__Vt(17), "ptr",this.__Value, "int",patternId, "ptr*",out))
		if name:=UIA_PatternId(patternId)
			return new UIA_%name%(out)
		return out
	}
	...
	GetParentHwnd() { ; Get the parent window hwnd from the element
		hwndNotZeroCond := this.__UIA.CreateNotCondition(this.__UIA.CreatePropertyCondition(UIA_PropertyId("NativeWindowHandle"), 0)) ; create a condition to find NativeWindowHandlePropertyId of not 0
		TW := this.__UIA.CreateTreeWalker(hwndNotZeroCond)
		try {
			hwnd := TW.NormalizeElement(this).GetCurrentPropertyValue(UIA_PropertyId("NativeWindowHandle"))
	...
	Click(but="", clickCount=1) { ; Click using LegacyIAccessible pattern DoDefaultAction(), or the Toggle pattern with Toggle() method or Invoke pattern Invoke() as fall-back, in these cases clickCount is ignored. If but is specified (for example "left", "right") then the native mouse Click function will be used to click the center of the element.
		;StringLower, but, but		
		if (but == "") {
			if (this.GetCurrentPropertyValue(UIA_IsInvokePatternAvailablePropertyId))
				this.GetCurrentPatternAs("Invoke").Invoke()
			else if (this.GetCurrentPropertyValue(UIA_IsTogglePatternAvailablePropertyId))
				this.GetCurrentPatternAs("Toggle").Toggle()
			else if (this.GetCurrentPropertyValue(UIA_IsExpandCollapsePatternAvailablePropertyId)) {
				if (this.GetCurrentPatternAs("ExpandCollapse").CurrentExpandCollapseState == 0)
					pattern.Expand()
				Else
					pattern.Collapse()
			} else if (this.GetCurrentPropertyValue(UIA_IsLegacyIAccessiblePatternAvailablePropertyId))
	...
	/*
		...
		Example1: "Name=Username:" would use FindFirst with UIA_PropertyId("Name") looking for the string "Username:"
		Example2: "ControlType=Button would FindFirst using UIA_PropertyId("ControlType") and UIA_ButtonControlTypeId. Alternatively "ControlType=50000" can be used (direct value for UIA_ButtonControlTypeId which is 50000)
	...
	*/
	FindFirstBy(expr, scope=0x4, matchMode=3, caseSensitive=True) { 
		...
		if (bufName[1]) {
			...
			nameCondition := (matchMode==1)?this.__UIA.CreatePropertyConditionEx(UIA_PropertyId("Name"), name,, 2|!caseSensitive):this.__UIA.CreateNotCondition(this.__UIA.CreatePropertyCondition(UIA_PropertyId("Name"), ""))
	...
	FindFirstByName(name, scope=0x4, matchMode=3, caseSensitive=True) { ; MatchMode has same convention as window TitleMatchMode: 1=needs to start with the specified name, 2=can contain anywhere, 3=exact match, RegEx=regex match
		if (matchMode == 3 || matchMode == 2) {
			;MsgBox, % "Find: " UIA_PropertyId("Name") " ControlType: " name
			nameCondition := this.__UIA.CreatePropertyConditionEx(UIA_PropertyId("Name"), name,, ((matchMode==3)?0:2)|!caseSensitive)
	...
	FindFirstByType(controlType, scope=0x4) {
		if controlType is not integer
			controlType := UIA_ControlTypeId(controlType)
		if !controlType
			throw Exception("Invalid control type specified", -1)
		controlCondition := this.__UIA.CreatePropertyCondition(UIA_PropertyId("ControlType"), controlType)
	...
	FindFirstByNameAndType(name, controlType, scope=0x4, matchMode=3, caseSensitive=True) {
		...
		if !controlType
			throw Exception("Invalid control type specified", -1)
		controlCondition := this.__UIA.CreatePropertyCondition(UIA_PropertyId("ControlType"), controlType, 3)
		if (matchMode == 3 || matchMode == 2) {
			nameCondition := this.__UIA.CreatePropertyConditionEx(UIA_PropertyId("Name"), name,, ((matchMode==3)?0:2)|!caseSensitive)
	...
	WaitElementExistByNameAndType(name, controlType, scope=0x4, caseSensitive=True, timeOut=10000) {
		startTime := A_TickCount
		while (!IsObject(el := this.FindFirstByNameAndType(name, controlType, scope, caseSensitive)) && ((timeOut < 1) ? 1 : (A_tickCount - startTime < timeOut))) {
			Sleep, 100
		}
		return el
	}
...
class UIA_Element7 extends UIA_Element6 {
	...
	GetCurrentMetadataValue(targetId, metadataId) {
		return UIA_Hr(DllCall(this.__Vt(114), "ptr",this.__Value, "int",targetId, "int", metadataId, "ptr*", UIA_Variant(out)))? new UIA_VariantData(out):
	}
}
...
class UIA_AndCondition extends UIA_Condition {
	...
	GetChildren() { ; Returns a native AHK array containing all the conditions as normal objects
...
class _UIA_EventHandler { ; This is replaced by UIA_CreateEventHandler
...
class _UIA_FocusChangedEventHandler { ; This is replaced by UIA_CreateEventHandler
...
class _UIA_PropertyChangedEventHandler { ; This is replaced by UIA_CreateEventHandler	
...
class _UIA_StructureChangedEventHandler { ; This is replaced by UIA_CreateEventHandler	
...
class _UIA_TextEditTextChangedEventHandler { ; This is replaced by UIA_CreateEventHandler
...
class UIA_StylesPattern extends UIA_Base { ; UNTESTED
	...
	GetCurrentExtendedPropertiesAsArray(byref propertyCount) {
		return UIA_Hr(DllCall(this.__Vt(10), "ptr",this.__Value, "ptr*", propertyArray, "int*", propertyCount))?ComObj(0x2003,propertyArray,1):
	}
	GetCachedExtendedPropertiesAsArray(byref propertyCount) {
		return UIA_Hr(DllCall(this.__Vt(18), "ptr",this.__Value, "ptr*", propertyArray, "int*", propertyCount))?ComObj(0x2003,propertyArray,1):
	}
}
...
class UIA_WindowPattern extends UIA_Base {
	...
	SetWindowVisualState(state) {
		return UIA_Hr(DllCall(this.__Vt(4), "ptr",this.__Value, "int",state))
	}
}
...
class UIA_DragPattern extends UIA_Base { ; UNTESTED, couldn't find a window that supported this
	...
	GetCurrentGrabbedItems() {
		return UIA_Hr(DllCall(this.__Vt(9), "ptr",this.__Value, "ptr*", UIA_Variant(out)))?UIA_VariantData(out):
	}
	GetCachedGrabbedItems() {
		return UIA_Hr(DllCall(this.__Vt(10), "ptr",this.__Value, "ptr*", UIA_Variant(out)))?UIA_VariantData(out):
	}
}
...
class UIA_TextRange extends UIA_Base {
	...
	GetBoundingRectangles() {
		return UIA_Hr(DllCall(this.__Vt(10), "ptr",this.__Value,"ptr",&(rect,VarSetCapacity(rect,16))))?new UIA_RectToObject(rect):
	}
	GetEnclosingElement() {
...
class UIA_TextRange3 extends UIA_TextRange2 { ; UNTESTED
	...
	GetAttributeValues(attributeIds, attributeIdCount) { ; currently returns a SafeArray
		...
		return UIA_Hr(DllCall(this.__Vt(24), "ptr",this.__Value, "ptr", ComObjValue(SafeArray), "int", attributeIdCount, "ptr*",out))? out:
	}
}
...
{  ;~ UIA Functions
	UIA_Interface(maxVersion="") {
		...
		if IsObject(uia)
			return uia
		max := (maxVersion?maxVersion:UIA_MaxVersion_Interface)+1
		while (--max) {
		...
		/* 
		for k, v in [[7,"{29de312e-83c6-4309-8808-e8dfcb46c3c2}"],[6,"{aae072da-29e3-413d-87a7-192dbf81ed10}"],[5,"{25f700c8-d816-4057-a9dc-3cbdee77e256}"],[4,"{1189c02a-05f8-4319-8e21-e817e3db2860}"],[3,"{73d768da-9b51-4b89-936e-c209290973e7}"],[2,"{34723aff-0c9d-49d0-9896-7ab52df8cd8a}"]] {
			try {
				ver := v[1]
				if uia:=ComObjCreate("{e22ad333-b25f-460c-83d0-0581107395c9}",v[2])
					return uia:=new UIA_Interface2(uia, 1, v[1]), uia.base.base.__UIA:=uia, uia.base.base.TrueCondition:=uia.CreateTrueCondition(), uia.base.base.TreeWalkerTrue := uia.CreateTreeWalker(uia.base.base.TrueCondition)
			}
		}
		try {
			if uia:=ComObjCreate("{ff48dba4-60ef-4201-aa87-54103eef594e}","{30cbe57d-d9d0-452a-ab13-7ac5ac4825ee}")
	...
	UIA_Element(e) {
		...
		if (previousVersion != UIA_Enum.CurrentVersion_Element) ; Check if the user wants an element with a different version
			v := ""
		else if v
			return (v==1)?new UIA_Element(e,,1):new UIA_Element%v%(e,,v)
		max := UIA_Enum.CurrentVersion_Element+1
		While (--max) {
	...
	UIA_TextRange(e) {
		static v
		max := UIA_MaxVersion_TextRange+1
		if v
	...
	UIA_SafeArrayToNativeArray(safearray) {
		out := []
		for k in safearray
			out.Push(k)
		return out
	...
	UIA_VariantData(ByRef p, flag=1) {
		/*
		return !(ret:=ComObject(0x400C, &p)[])?"Invalid Variant"
				:(vt:=NumGet(p,"UShort"))=11?-ret ; Account for VT_BOOL being inverted
				:ret
		*/
	...
	*/
	...
	UIA_CreateEventHandler(name){
		if !IsFunc(name){
			msgbox %name% is not a function.
			return
		}
		static ptr
...
{ ;~ UIA CONSTANTS
	...
	UIA_PropertyId(n="") {
		...
		RegexMatch(StrReplace(StrReplace(ids, "UIA_"), "PropertyId"), "(?:^|,)" n "(?:" n ")?(?:Id)?:(\d+)", m)
		return m1
	}
...
class UIA_Enum { ; source: https://github.com/Ixiko/AHK-libs-and-classes-collection/blob/master/libs/o-z/UIAutomationClient_1_0_64bit.ahk

	; UIA_Interface specific enums
	static MaxVersion_Interface := 7
	static MaxVersion_Element := 7
	static CurrentVersion_Element := 7
	static MaxVersion_TextRange := 3
